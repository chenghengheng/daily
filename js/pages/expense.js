const Expense = {
  ...
  render(container) {
    ...
    const quickTotal = current.filter(item => item.source !== 'wish').reduce((sum, item) => sum + item.amount, 0);
    const wishTotal = current.filter(item => item.source === 'wish').reduce((sum, item) => sum + item.amount, 0);
