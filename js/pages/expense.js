const Expense = {
  ...
  render(container) {
    ...
    container.querySelectorAll('[data-delete]').forEach(button => button.onclick = () => { const item = this.items.find(value => value.id === button.dataset.delete); if (!item) return; item.deletedAt = new Date().toISOString(); item.updatedAt = item.deletedAt; Store.saveExpenses(this.items); this.render(container); Toast.show('已删除，可在页面顶部撤销'); });
